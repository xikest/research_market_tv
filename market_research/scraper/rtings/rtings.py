...
# 바이플롯 그리기
plt.scatter(df['PC1'], df['PC2'], marker='o', color='blue')

# 로딩 벡터에 대한 화살표 추가
for i, feature in enumerate(df.columns[:-2]):  # 마지막 두 열은 PC1과 PC2이므로 제외
    plt.arrow(0, 0, loading_vectors[i, 0], loading_vectors[i, 1], color='red', width=0.05)
    plt.text(loading_vectors[i, 0], loading_vectors[i, 1], feature, color='red', ha='right', va='bottom')

# 그래프에 레이블 추가
plt.xlabel('PC1')
plt.ylabel('PC2')
plt.title('PCA Biplot with Loading Vectors')
